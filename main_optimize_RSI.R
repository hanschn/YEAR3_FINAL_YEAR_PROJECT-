source('framework/data.R'); 
source('framework/backtester.R')
#source('framework/processResults.R'); 
#source('strategies/bbands_contrarian.R') 
source('strategies/RSI_EMA.R')
numOfDays <- 500
dataList <- getData(directory="PART1")
dataList <- lapply(dataList, function(x) x[1:numOfDays])
sMult <- 0.2 # slippage multiplier

#lookbackSeq <- seq(from=20,to=40,by=10)
#sdParamSeq  <- seq(from=1.5,to=2,by=0.5) 
#paramsList  <- list(lookbackSeq,sdParamSeq)
#numberComb <- prod(sapply(paramsList,length))
rsithresholdSeq <- seq(from=40,to=60,by=10)
emalongSeq <- seq(from=20,to=30,by=5) 
emashortSeq <- seq(from=5,to=15,by=5)
ratioSeq <- seq(from=0.1,to=0.3,by=0.1)
paramsList  <- list(rsithresholdSeq,emalongSeq,emashortSeq,ratioSeq)
numberComb <- prod(sapply(paramsList,length))



#resultsMatrix <- matrix(nrow=numberComb,ncol=3)
#colnames(resultsMatrix) <- c("lookback","sdParam","PD Ratio")
#pfolioPnLList <- vector(mode="list",length=numberComb)
resultsMatrix <- matrix(nrow=numberComb,ncol=5)
colnames(resultsMatrix) <- c("rsi_threshold","emalong","emashortSeq","ratioSeq","PD Ratio")
pfolioPnLList <- vector(mode="list",length=numberComb)



#count <- 1
#for (lb in lookbackSeq) {
#    for (sdp in sdParamSeq) {
#        params <- list(lookback=lb,sdParam=sdp,series=1:4,posSizes=rep(1,10)) 
#        results <- backtest(dataList, getOrders, params, sMult)
#        pfolioPnL <- plotResults(dataList,results)
#        resultsMatrix[count,] <- c(lb,sdp,pfolioPnL$fitAgg)
#        pfolioPnLList[[count]]<- pfolioPnL
#        cat("Just completed",count,"out of",numberComb,"\n")
#        print(resultsMatrix[count,])
#        count <- count + 1
#    }
#}
count <- 1
for (rt in rsithresholdSeq) {
    for (els in emalongSeq) {
        for(ess in emashortSeq){
            for(rse in ratioSeq){
        params <- list(rsi_threshold=rt,ema_long=els,ema_short=ess, rsi_period = 14,ratio = rse) 
        results <- backtest(dataList, getOrders, params, sMult)
        pfolioPnL <- plotResults(dataList,results)
        resultsMatrix[count,] <- c(rt,els,ess,rse,pfolioPnL$fitAgg)
        pfolioPnLList[[count]]<- pfolioPnL
        cat("Just completed",count,"out of",numberComb,"\n")
        print(resultsMatrix[count,])
        count <- count + 1
            }
        }
    }
}
print(resultsMatrix[order(resultsMatrix[,"PD Ratio"]),])
